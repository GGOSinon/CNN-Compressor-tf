...
with sess:
    ...
    for i in range(n_repeat):
        ...
        while step * batch_size < training_iters:
            ...
            feed_dict_cor = {img_input: batch_x, img_input_cor: img_cor_decom, img_ans: batch_y} 
            for _ in range(gen_train):
                #print(img_compressed.shape, img_compressed.dtype)
            	sess.run(cor_op, feed_dict=feed_dict_cor) 
            if step % display_step == 0:
                iloss = sess.run(img_loss, feed_dict=feed_dict)
                rloss = sess.run(cor_loss, feed_dict=feed_dict_cor)
                #iloss /= batch_size
                #rloss /= batch_size
                print ("Step " + str(i) + ", Iter " + str(step*batch_size) + ", Minibatch ILoss= " + "{:.6f}".format(iloss) + ", Minibatch RLoss= " + "{:.6f}".format(rloss))

                valid_x, valid_y = valid_batch(0, n_valid)
                feed_dict = {img_input:valid_x, img_ans:valid_y}
                val_iloss = sess.run(img_loss, feed_dict=feed_dict)
                img_cor_prob, img_fin = sess.run([img_prob, img_final], feed_dict=feed_dict)
                img_inc, img_val = cor_compress(valid_x, img_cor_prob)
                img_cor_decom = cor_decompress(img_inc, img_val, img_fin)
                feed_dict_cor = {img_input: valid_x, img_input_cor: img_cor_decom, img_ans: valid_y}
            
                val_rloss = sess.run(cor_loss,feed_dict=feed_dict_cor)
                #tot_iloss /= n_valid
                #tot_rloss /= n_valid
                print ("Testing ILoss : %.5f, RLoss : %.5f" % (val_iloss, val_rloss))
                Log = open('Log.txt', 'a')
                Log.write("I: "+"{:.5f}".format(val_iloss)+" R: "+"{:.5f}".format(val_rloss)+"\n")
                Log.close()
            if step%decay_step==0:ir_learning_rate *= ir_decay_rate
            step += 1

        saver.save(sess, './model/CNN-model.ckpt', global_step=i)
        #learning_rate
        #learning_rate/=1.4
